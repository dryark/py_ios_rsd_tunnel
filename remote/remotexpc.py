...
class RemoteXPCConnection:
    ...
    def send_request(
        ...
        xpc_wrapper = create_xpc_wrapper(
            data,
            message_id=self.next_message_id[ROOT_CHANNEL],
        ...
        while True:
            ...
            if frame.stream_id % 2 == 0 and frame.body_len > 0:
                self._send_frame(
                    WindowUpdateFrame(
                        stream_id=0,
                        window_increment=frame.body_len
                    )
                )
    ...
    def _receive_frame(self) -> Frame:
        ...
        frame, additional_size = Frame.parse_frame_header(
            memoryview(buf)
        )
        
        frame.parse_body(
            memoryview(
                self._recvall(additional_size)
            )
        )
        return frame
